...
resource "aws_iam_role_policy_attachment" "AmazonEKSClusterPolicy" {
  policy_arn = "arn:aws:iam::aws:policy/devsecopsAmazonEKSClusterPolicy"
  role       = aws_iam_role.devsecopsEKSClusterRole.name
}

# NODE GROUP ROLE
resource "aws_iam_role" "NodeGroupRole" {
  name = "devsecopsEKSNodeGroupRole"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "AmazonEKSWorkerNodePolicy" {
  policy_arn = "arn:aws:iam::aws:policy/devsecopsAmazonEKSWorkerNodePolicy"
  role       = aws_iam_role.devsecopsNodeGroupRole.name
}

// Provides read-only access to Amazon EC2 Container Registry repositories.
resource "aws_iam_role_policy_attachment" "AmazonEC2ContainerRegistryReadOnly" {
  policy_arn = "arn:aws:iam::aws:policy/devsecopsAmazonEC2ContainerRegistryReadOnly"
  role       = aws_iam_role.devsecopsNodeGroupRole.name
}
...
resource "aws_iam_role_policy_attachment" "AmazonEKS_CNI_Policy" {
  policy_arn = "arn:aws:iam::aws:policy/devsecopsAmazonEKS_CNI_Policy"
  role       = aws_iam_role.devsecopsNodeGroupRole.name
}
